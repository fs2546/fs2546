...
class EmployeeData:
  ...
  def __parse_file(self,file):
    ...
    for line in open(file):
      ...
      data.append((int(employee_id), int(income)))
    return data
...
class Calculator:
  ...
  def calculate(self, data_item):
    ...
    if income < self.config.jishu_low:
      shebao = self.config.jishu_low * self.config.total_rate
    elif income > self.config.jishu_high:
      shebao = self.config.jishu_high * self.config.total_rate
    else:
      shebao = income * self.config.total_rate
      resid_income = income - shebao
    tax_income = resid_income - self.tax_start

    if tax_income < 0: 
      tax = 0
    else:
      for item in self.tax_table:
        if tax_income > item[0]:
          tax = tax_income * item[1] - item[2]
          break
    
    final_income = resid_income - tax
    return str(employee_id), str(income), '{:.2f}'.format(shebao),'{:.2f}'.format(tax), '{:.2f}'.format(final_income)
